...
public class JSONObject {
     ...
     */
    public JSONObject(JSONObject jsonObj, String[] names) {
        this();
     ...
     *
     * @param token A JSONTokener object containing the source string.
     * @throws JSONException If there is a syntax error in the source string or a
     *                       duplicated key.
     */
    public JSONObject(org.json.JSONTokener token) throws JSONException {
        this();
        char character;
        String key;
...
// The key is followed by ':'.
            ...
            this.putOnce(key, token.nextValue());
...
// Pairs are separated by ','.
     ...
     * @param bean object that has getters that should be used to make JSONObject
     */
    ...
    public JSONObject(Object object, String names[]) {
        ...
        for (int i = 0; i < names.length; i += 1) {
            String name = names[i];
            try {
                this.putOpt(name, classObj.getField(name).get(object));
            } catch (Exception ignore) {}
        }
    }
     ...
     * is stored under the key to hold all of the accumulated values. In contrast,
     * the put method replaces the previous value. If only one value is accumulated
    ...
    public JSONObject accumulate(String key, Object value) throws JSONException {
        ...
        if (object == null) {
            this.put(key, value instanceof org.json.JSONArray ? new org.json.JSONArray().put(value) : value);
     ...
     */
    public static String[] getNames(JSONObject jsonObj) {
...
// Shave off trailing zeros and decimal point, if possible.
    ...
    public org.json.JSONArray optJSONArray(String key) {
        ...
        return obj instanceof org.json.JSONArray ? (org.json.JSONArray) obj : null;
    }
            ...
            switch (character) {
                ...
                default:
                    if ((character < ' ') || ((character >= '\u0080') && (character < '\u00a0')) || ((character >= '\u2000') && (character < '\u2100'))) {
                        writer.write("\\u");
                        hexString = Integer.toHexString(character);
    ...
    public org.json.JSONArray toJSONArray(org.json.JSONArray names) throws JSONException {
        ...
        org.json.JSONArray jsonArray = new org.json.JSONArray();
        for (int i = 0; i < names.length(); i += 1) {
            jsonArray.put(this.opt(names.getString(i)));
        }
        return jsonArray;
    }
     ...
     * required to produce a strictly conforming text. If the object does not contain a
     * toJSONString method (which is the most constants case), then a text will be produced
     * by other means. Warning: This method assumes that the data structure is acyclical.
    ...
    /**
     * Used to point at the current JSONObject when doing queryObject iteration function.
     */
    private JSONObject currentQueriedObject = this;

    /**
     * Set currentQueriedObject to the JSONObject itself for the first query
     */
    private boolean firstQuery = true;

    /**
     * Query JSONObject using keys ArrayList for easier retrieval
     * @param keys JSONObject path keys
     * @return the queried object
     */
    public JSONObject queryObject(ArrayList<String> keys) {
        if (keys.size() == 0) {
            return null;
        }
        ...
        try {
            if (firstQuery) {
                currentQueriedObject = this;
            }

            if (keys.size() == 1) {
                currentQueriedObject = currentQueriedObject.getJSONObject(keys.get(0));
                firstQuery = true;
                return currentQueriedObject;
            } else {
                firstQuery = false;
                String currentKey = keys.get(0);
                keys.remove(0);
                currentQueriedObject = currentQueriedObject.getJSONObject(currentKey);
                return queryObject(keys);
            }
        }
        catch (Exception ex)
        {
