/*
 * Copyright (c) 2019, HP Development Company, L.P. All rights reserved.
 * This software contains confidential and proprietary information of HP.
 * The user of this software agrees not to disclose, disseminate or copy
 * such Confidential Information and shall use the software only in accordance
 * with the terms of the license agreement the user entered into with HP.
 */

package com.creditSuisse.utility.system.information;

/**
 * system HPA related information
 * @author Edward Guo
 * @since 11/16/2019
 */
public class SystemInformation {
    public String releaseCycle;
    public String hardwareType;
    public String buildId;
    public String productLineCode;
    public String productName;
    public String internalPlatformName;
    public String internalPlatformVersion;
    public String segment;
    public String regionShipped;
    public String featureByte;
}
